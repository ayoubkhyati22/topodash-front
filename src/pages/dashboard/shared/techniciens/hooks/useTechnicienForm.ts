// hooks/useTechnicienForm.ts
import { useState, useCallback } from 'react';
import { TechnicienCreateRequest, ApiResponse } from '../types';
import { useAuth } from '../../../../../AuthContext';
import { City } from 'pages/dashboard/types';

interface UseTechnicienFormReturn {
  loading: boolean;
  error: string | null;
  success: boolean;
  cities: City[];
  ...
  createTechnicien: (data: TechnicienCreateRequest) => Promise<boolean>;
  fetchCities: () => Promise<void>;
  fetchTopographes: () => Promise<void>;
  resetForm: () => void;
}

export const useTechnicienForm = (): UseTechnicienFormReturn => {
  const { user } = useAuth();
  ...
  const createTechnicien = useCallback(async (data: TechnicienCreateRequest): Promise<boolean> => {
    if (!user?.token) {
    ...
    try {
      ...
      // Si l'utilisateur est un topographe, ne pas envoyer le assignedToTopographeId
      // car l'affectation sera automatique côté backend
      const requestData: { assignedToTopographeId?: number; [key: string]: any } = { ...data };
      if (user.role === 'TOPOGRAPHE') {
        delete requestData.assignedToTopographeId;
      }

      const response = await fetch('http://localhost:8080/api/technicien', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${user?.token}`,
